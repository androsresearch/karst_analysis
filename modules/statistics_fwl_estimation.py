...
def load_and_filter_data(
    ...
    The file_info_df must contain the columns:
        ...
        - 'vp_lrst': Filter value for method LRST.
    ...
    Returns:
        ...
        and the second key is the filtering method (e.g. 'IC', 'BIC', 'DGH', 'LRST'), and the value is 
...
def compute_statistics(filtered_data: dict, file_info_df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    filtered_data : dict
        ...
        dictionaries with method names ('IC', 'BIC', 'DGH', 'LRST') as keys and DataFrames as values.
    file_info_df : pd.DataFrame
    ...
    pd.DataFrame
        A DataFrame with the following columns:
         ...
         - method: Filtering method used (e.g. IC, BIC, DGH, LRST)
    ...
    for well_id, methods in filtered_data.items():
        ...
        for method, df in methods.items():
            ...
            if method == "DGH":
                ...
                vp_selected = vp_row["vp_lrst"].values[0]
            else:
...
#     The legend groups and labels are set by method (e.g., 'IC', 'BIC', 'DGH', 'LRST'), 
...
#             and the second key is the filtering method ('IC', 'BIC', 'DGH', 'LRST', etc.).
#         variable (str): 
...
def generate_boxplots_matplotlib(
    ...
    Args:
        filtered_data (Dict[str, Dict[str, pd.DataFrame]]):
            ...
            - First key: Well ID
            - Second key: Filtering method (e.g., 'IC', 'BIC', 'DGH', 'LRST').
        variable (str):
            Name of the column to be plotted (e.g., "Corrected sp Cond [uS/cm]" or "Vertical Position [m]").
        file_info_df (pd.DataFrame):
            DataFrame containing well information, including fwl_thickness values for each method.
            Must contain columns 'ID', 'vp_ic', 'vp_bic', 'vp_dgh', 'vp_lrst'.
        show_outliers (bool):
            Whether to display outliers in the boxplots.
        order (Optional[List[str]]):
        ...
        methods_to_show (Optional[List[str]]):
            List of filtering methods to display (e.g., ['IC', 'BIC', 'DGH', 'LRST']).
    ...
    # Method to column name mapping for thickness values
    method_to_column = {
        'IC': 'vp_ic',
        'BIC': 'vp_bic',
        'DGH': 'vp_dgh',
    ...
    # 5) Add annotations to the right side indicating 'n', 'out', and 'thickness'
    for i, (data, well_id, method) in enumerate(zip(group_data, well_id_list, method_list)):
        n_points = len(data)
        n_outliers = calculate_outliers(data)
        
        # Get the thickness value from file_info_df
        thickness_col = method_to_column.get(method)
        thickness_value = None
        if thickness_col is not None:
            well_row = file_info_df[file_info_df['ID'] == well_id]
            if not well_row.empty and thickness_col in well_row.columns:
                thickness_value = well_row[thickness_col].values[0]
        
        # Create annotation text with thickness value if available
        if thickness_value is not None:
            annotation_text = f"n={n_points}, out={n_outliers}, t={thickness_value:.2f}"
        else:
            annotation_text = f"n={n_points}, out={n_outliers}"
